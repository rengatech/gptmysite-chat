...
export class SidebarUserDetailsComponent implements OnInit, OnChanges {
  ...
  teammateStatus = [
    {
      id: 1,
      name: "Available",
      avatar: "assets/images/teammate-status/avaible.svg",
      label: "LABEL_AVAILABLE",
    },
    {
      id: 2,
      name: "Unavailable",
      avatar: "assets/images/teammate-status/unavaible.svg",
      label: "LABEL_NOT_AVAILABLE",
    },
    {
      id: 3,
      name: "Inactive",
      avatar: "assets/images/teammate-status/inactive.svg",
  ...
  ngOnInit() {
    this.DASHBOARD_URL =
      this.appConfigProvider.getConfig().dashboardUrl + "#/project/";
    ...
    this.messagingAuthService.BSAuthStateChanged.subscribe((state) => {
      ...
      if (state === "online") {
        ...
        if (storedCurrentUser && storedCurrentUser !== "undefined") {
          ...
          this.logger.log(
            "[SIDEBAR-USER-DETAILS] - subcribeToAuthStateChanged - currentUser ",
            currentUser
          );
          if (currentUser) {
            ...
            this.photo_profile_URL = this.imageRepoService.getImagePhotoUrl(
              this.user.uid
            );
            this.logger.log(
              "[SIDEBAR-USER-DETAILS] photo_profile_URL ",
              this.photo_profile_URL
            );
            this.checkIfExistPhotoProfile(this.photo_profile_URL);
          }
        } else {
          this.logger.error(
            "[SIDEBAR-USER-DETAILS] currentUser not found in storage "
          );
        }
      }
    });
  ...
  checkIfExistPhotoProfile(imageUrl) {
    this.verifyImageURL(imageUrl, (imageExists) => {
      if (imageExists === true) {
        ...
        this.logger.log(
          "[SIDEBAR-USER-DETAILS] photo_profile_URL IMAGE EXIST ",
          imageExists
        );
      } else {
  ...
  getCurrentChatLangAndTranslateLabels(currentUser) {
    ...
    this.logger.log(
      "[SIDEBAR-USER-DETAILS] - ngOnInit - currentUser ",
      currentUser
    );
    this.logger.log(
      "[SIDEBAR-USER-DETAILS] - ngOnInit - browserLang ",
      this.browserLang
    );

    const stored_preferred_lang = localStorage.getItem(
      currentUser.uid + "_lang"
    );
    this.logger.log(
      "[SIDEBAR-USER-DETAILS] stored_preferred_lang: ",
    ...
    if (tranlatedLanguage.includes(this.chat_lang)) {
      this.logger.log(
        "[SIDEBAR-USER-DETAILS] tranlatedLanguage includes",
        this.chat_lang,
        ": ",
        tranlatedLanguage.includes(this.chat_lang)
      );
  ...
  translateLabels() {
    ...
    this.translate.get(keys).subscribe((text: string) => {
      this.translationsMap
        .set("LABEL_AVAILABLE", text["LABEL_AVAILABLE"])
  ...
  getOSCODE() {
    this.public_Key =
      this.appConfigProvider.getConfig().t2y12PruGU9wUtEGzBJfolMIgK;
    this.logger.log(
      "[SIDEBAR-USER-DETAILS] AppConfigService getAppConfig public_Key",
      this.public_Key
    );
    this.logger.log(
      "[SIDEBAR-USER-DETAILS] AppConfigService getAppConfig",
      this.appConfigProvider.getConfig()
    );
    if (this.public_Key) {
      let keys = this.public_Key.split("-");
      this.logger.log(
        "[SIDEBAR-USER-DETAILS] PUBLIC-KEY - public_Key keys",
        keys
      );
      ...
      if (projectObjct && projectObjct !== "undefined") {
        ...
        this.project = {
          ...
          isActiveSubscription:
            projectObjct["id_project"]["isActiveSubscription"],
        ...
        if (this.project.profile.type === "free") {
          ...
          this.project.profile.type === "payment" &&
          this.project.profile.name === "pro"
        ) {
          this.getProPlanTranslation();
        } else if (
          this.project.profile.type === "payment" &&
          this.project.profile.name === "enterprise"
        ) {
  ...
  listenTocurrentProjectUserUserAvailability$() {
    this.wsService.currentProjectUserAvailability$.pipe(skip(1)).subscribe(
      (projectUser) => {
        this.logger.log(
          "[SIDEBAR-USER-DETAILS] - $UBSC TO WS USER AVAILABILITY & BUSY STATUS RES ",
          projectUser
        );

        if (projectUser) {
          if (
            projectUser["user_available"] === false &&
            projectUser["profileStatus"] === "inactive"
          ) {
            // console.log('teammateStatus ', this.teammateStatus)
            this.selectedStatus = this.teammateStatus[2].id;
            this.logger.debug(
              "[SIDEBAR-USER-DETAILS] - PROFILE_STATUS selected option",
            ...
            projectUser["user_available"] === true &&
            (projectUser["profileStatus"] === "" ||
              !projectUser["profileStatus"])
          ) {
      ...
      (error) => {
        this.logger.error(
          "[SIDEBAR-USER-DETAILS] - $UBSC TO WS USER AVAILABILITY & BUSY STATUS error ",
          error
        );
      },
      () => {
        this.logger.log(
          "[SIDEBAR-USER-DETAILS] - $UBSC TO WS USER AVAILABILITY & BUSY STATUS * COMPLETE *"
        );
      }
    );
  }
  ...
  changeAvailabilityStateInUserDetailsSidebar(selectedStatusID) {
    this.logger.log(
      "[SIDEBAR-USER-DETAILS] - changeAvailabilityState projectid",
      this.project._id,
      " available 1: ",
      selectedStatusID
    );
    ...
    this.wsService
      ...
      .subscribe(
        (projectUser: any) => {
          this.logger.log(
            "[SIDEBAR-USER-DETAILS] - PROJECT-USER UPDATED ",
            projectUser
          );
        },
        (error) => {
          this.logger.error(
            "[SIDEBAR-USER-DETAILS] - PROJECT-USER UPDATED - ERROR  ",
            error
          );
        },
        () => {
          this.logger.log(
            "[SIDEBAR-USER-DETAILS] - PROJECT-USER UPDATED  * COMPLETE *"
          );
        }
      );
  }
